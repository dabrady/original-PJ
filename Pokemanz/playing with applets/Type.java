// package pokemanz;
import java.util.*;
//Change print statements in getMultipliers
...
public abstract class Type {
  ...
  String name;
  String[] weakAgainst; // YOU are weak against THEM
  String[] resistantTo;  //YOU are resistant to THEM
  String[] unaffectedBy;  //YOU are unaffected by THEM
  
  Type(){}
  
  public double getMultipliers (Type t) {
	System.out.println("When asked for his multipliers concerning the elemental type '" + t.name + "', this pokemon replies:\n");
	
    for (int i = 0; i < this.weakAgainst.length; i++) {
	  if (this.weakAgainst[i].equals(t.name)) {
	    //System.out.println("Pssht! Just a flesh wound!");
		return 2;
	  }
	}

	for (int i = 0; i < this.resistantTo.length; i++) {
	  if (this.resistantTo[i].equals(t.name)) {
	    //System.out.println("Bah! 'Tis only a scratch!");
		return 0.5;
	  }
	}

	for (int i = 0; i < this.unaffectedBy.length; i++) {
	  if (this.unaffectedBy[i].equals(t.name)) {
	    //System.out.println("Hehe...that tickles!");
		return 0;
	  }
	}
	System.out.println("You can take 'em!");
	return 1;
  }
  ...
  public static void main(String[] args){	
	...
	System.out.println(t.getMultipliers(s));
	System.out.println("**************************************");
	System.out.println(s.getMultipliers(t));
  }
}
